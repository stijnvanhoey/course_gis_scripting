...
Other (geometric) operations are provided by (a huge amount of) additional R Packages. The `rgeos` package is n important package to check for these functionalities. As an example: adding a buffer to our `SpatialPoints` object:
```{r}
...
ggplot(pts_recorded_df, aes(x = longitude, y = latitude)) +
    geom_point() +
    geom_text(size = 2, aes(label = scientific_names), 
